...
const HOME_COPY = {
  pl: {
    ...
    jewelryItems: [
      "Ręcznie wykonane pierścionki, kolczyki, wisiorki i bransoletki",
      "Kamienie naturalne i metale szlachetne",
      "Indywidualne projekty biżuterii",
      "Tradycyjne rzemiosło spotyka modelowanie 3D",
    ],
    studioListTitle: "AEJaCA sTuDiO",
    studioItems: [
      "Projektowanie 3D (CAD) i druk 3D",
      "Grawer laserem światłowodowym i CO₂",
      "Odlewanie żywicą (UV i dwuskładnikowa)",
      "NFC, smart tagi, niestandardowe gadżety",
    ],
  ...
  de: {
    ...
    jewelryListTitle: "AEJaCA Jewelry",
    jewelryItems: [
      "Handgefertigte Ringe, Ohrringe, Anhänger und Armbänder",
      "Naturedelsteine und Edelmetalle",
      "Individuelle Schmuckprojekte",
      "Traditionelles Handwerk trifft 3D-Modellierung",
    ],
    studioListTitle: "AEJaCA sTuDiO",
    studioItems: [
      "3D-Design (CAD) und 3D-Druck",
      "Faser- und CO₂-Lasergravur/-schneiden",
      "Epoxidharzguss (UV und zweikomponentig)",
      "NFC-Geräte, Smart Tags und Sondergadgets",
    ],
  ...
  return (
    <div>
      ...
      <section style={{ background: "var(--bg-page)", padding: "56px 16px 32px", textAlign: "center" }}>
        ...
        <p style={{ color: "var(--fg-2)", fontSize: 18, marginTop: 18, maxWidth: 640, marginLeft: "auto", marginRight: "auto", lineHeight: 1.55 }}>{t.sub}</p>
        <div style={{ marginTop: 24 }}><RatingPill rating={4.9} count={127} label="Google reviews" /></div>
      </section>

      {/* Two gateway tiles */}
      <section style={{ background: "var(--bg-page)", padding: "20px 16px 56px" }}>
        <div style={{ maxWidth: 960, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          ...
          <GatewayTile
            ...
            desc={lang === "pl" ? "Druk 3D, grawer laserowy, frezowanie CNC. Wycena online w jednej chwili."
              : lang === "de" ? "3D-Druck, Lasergravur, CNC-Fräsen. Sofortangebot online."
              : "3D printing, laser engraving, CNC milling. Get an instant quote online."}
      ...
      {/* Brand statement */}
      <section style={{ background: "var(--bg-page)", padding: "80px 16px", textAlign: "center" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <img
            src="../../assets/brand-sign.webp"
            alt="AEJaCA brand mark"
            style={{
              width: 144, height: 144, display: "block", margin: "0 auto 28px",
              filter: "brightness(0) invert(1) drop-shadow(0 0 20px rgba(255,255,255,0.15))",
              opacity: 0.85,
            }}
          />
          <h2 style={{ fontFamily: "var(--font-serif)", fontSize: 36, fontWeight: 600, margin: "0 0 18px", color: "#fff", lineHeight: 1.15 }}>{t.brandHeading}</h2>
          <p style={{ color: "var(--fg-3)", fontSize: 18, lineHeight: 1.6, margin: 0 }}>{t.brandText}</p>
          <div style={{ marginTop: 22 }}><RatingPill rating={4.9} count={127} /></div>
        </div>
      </section>
      ...
      {/* Quick quote */}
      <section style={{ padding: "80px 16px", background: "linear-gradient(180deg, var(--bg-page), rgba(23,23,23,0.30), var(--bg-page))" }}>
        <div style={{ maxWidth: 960, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            <Eyebrow variant="emerald" icon={<Zap size={14} />} style={{ marginBottom: 14 }}>{t.quickQuoteTag}</Eyebrow>
            <h2 style={{ fontFamily: "var(--font-serif)", fontSize: 36, fontWeight: 600, color: "#fff", margin: "0 0 14px", lineHeight: 1.15 }}>{t.quickQuoteTitle}</h2>
            <p style={{ color: "var(--fg-3)", fontSize: 16, maxWidth: 640, margin: "0 auto", lineHeight: 1.55 }}>{t.quickQuoteSub}</p>
          </div>
          ...
          {/* Emerald STL/SVG banner */}
          <div style={{
            borderRadius: 16, padding: 32, overflow: "hidden",
            border: "1px solid var(--border-emerald)",
            background: "linear-gradient(90deg, rgba(6,78,59,0.25), rgba(23,23,23,0.5), rgba(6,78,59,0.25))",
          }}>
            <div style={{ display: "flex", alignItems: "center", gap: 24, marginBottom: 22, flexWrap: "wrap" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ width: 48, height: 48, borderRadius: 12, background: "rgba(52,211,153,0.10)", border: "1px solid var(--border-emerald)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <FileUp size={22} style={{ color: "var(--emerald-400)" }} />
                </div>
                <span style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: "0.18em", color: "rgba(52,211,153,0.70)" }}>{t.stlTag}</span>
              </div>
              <div style={{ flex: 1, minWidth: 200 }}>
                <h3 style={{ fontFamily: "var(--font-sans)", fontSize: 18, fontWeight: 600, color: "#fff", margin: "0 0 4px" }}>{t.stlTitle}</h3>
                <p style={{ color: "var(--fg-3)", fontSize: 14, margin: 0, lineHeight: 1.55 }}>{t.stlDesc}</p>
              </div>
            </div>
      ...
      {/* Two Worlds glass cards */}
      <section style={{ padding: "64px 16px", background: "rgba(23,23,23,0.50)" }}>
        <div style={{ maxWidth: 1024, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 28 }}>
          <GlassFeatureCard
            eyebrow={t.twoWorldsAmber}
            title={t.jewelryListTitle}
            items={t.jewelryItems}
            ctaLabel={t.learnMoreJewelry}
            accent="amber"
            serif
            onClick={() => setActivePage("jewelry")}
          />
          <GlassFeatureCard
            eyebrow={t.twoWorldsBlue}
            title={t.studioListTitle}
            items={t.studioItems}
            ctaLabel={t.learnMoreStudio}
            accent="blue"
            onClick={() => setActivePage("studio")}
          />
        </div>
      ...
      <section style={{ padding: "80px 16px", background: "var(--bg-page)", textAlign: "center" }}>
        <h2 style={{ fontFamily: "var(--font-serif)", fontSize: 28, fontWeight: 600, margin: "0 0 14px", color: "#fff" }}>{t.ctaHeading}</h2>
        <p style={{ color: "var(--fg-3)", margin: "0 auto 28px", maxWidth: 480 }}>{t.ctaText}</p>
