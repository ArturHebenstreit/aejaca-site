...
| Stan "wybrane" niesiony kolorem: mgla na zdjeciu, kolorowa etykieta i pierscien, w czterech kolorach i trzydziestu miejscach (2026-09-02) | kazdy kalkulator dostal swoj kolor przy powstaniu, nikt nie ogladal ich obok siebie; roz w kalkulatorze bizuterii i zielen w sTuDiO nie sa nawet kolorami marki | jedno zrodlo `.kafelek-wybrany` w `index.css` i token `--ds-wybor`; zaznaczenie jest bezbarwne, wiec nie da sie go "zepsuc kolorem" |
| Podniesienie tla karty zabilo kontrast tekstu, ktory wczesniej przechodzil | tlo i tekst zmienia sie w osobnych plikach, a prog liczy sie z ich pary | `dostepnosc.mjs` po KAZDEJ zmianie koloru tla, nie tylko tekstu; `text-neutral-500` (#808080) nie przechodzi na podniesionej karcie w ciemnym (4,03), `text-neutral-400` przechodzi wszedzie |
