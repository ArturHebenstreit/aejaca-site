...
//
// Domyslnie pozycje trafiaja do bazy jako szkice. Sklep pokazywalby
...
// stan magazynowy i stan pozycji takie, jakie ustawiono w panelu.
...
for (const p of PRODUCT_SEED) {
  const { rows } = await pool.query(
    `INSERT INTO products
       (slug, kind, category, subcategory, offer, status, title, short, description, specs, note,
    ...
    [
      p.slug, p.kind, p.category, p.subcategory || null, p.offer, activate ? "live" : "draft",
  ...
  console.log(`  ${rows[0].slug}  ${rows[0].status}`);
}

console.log(`[produkty] wgrano ${inserted} pozycji`);
if (!activate) console.log("[produkty] pozycje sa szkicami, wystaw je w panelu albo uruchom z --activate");
await pool.end();
