...
**Precious-metal casting product, Build 1.008:** route 1 is casting a supplied ready wax or castable-resin pattern; route 2 is printing and casting an uploaded STL/OBJ/3MF/STEP/STP model; route 3 is CAD, pattern and casting from the customer's idea. All routes support Ag 800/925 and Au 9k/14k/18k, using AEJaCA or supplied metal. Pure gold (24k) is offered for other jewellery work but is NOT cast: it is too soft for a wearable piece and behaves differently in the flask (NIE_DO_ODLEWU in jewelryConfig.js). Only route 2 with AEJaCA metal can receive an automatic binding price, because volume is measured server-side. Casting flask: RADIANCE3.5, perforated, 83 mm inner diameter x 100 mm deep. Automatic casting limit: the model must fit a circle of 63 mm and 62 mm of height after orientation and the selected uniform scale. The circle is the flask minus 10 mm of investment against the wall on each side, so what counts is the DIAGONAL of the model's footprint, not each axis on its own: a flat piece 59 x 22 mm has a 63.0 mm diagonal and fits. The height is the flask minus a 20 mm sprue base with the button and 20 mm of investment above the model, which keeps the mould from cracking during burnout, plus 2 mm gained from the tilt used to lay the model in the flask. A long thin piece can also lie down across the flask, so a 62 mm piece fits. Finishing has five levels, each dearer than the last: as-cast with sprues (no surcharge), as-cast with sprues removed (+30 PLN), sprues cut and dressed flush (+70 PLN), ground (+110 PLN), jewellery finish meaning grinding and polishing (+160 PLN). The customer can return from the technical fit to original scale, which then requires individual review if it exceeds the limit. Scaling also changes every wall, prong and channel, so workshop validation remains mandatory. Physical patterns, idea-only work, models left over the limit and supplied metal need a human quote. For supplied metal explain a typical 5-8% technological loss for clean single-alloy stock and 10-15% for mixed or uncertain stock; plated or filled material may be refused. The same service card also sits in the jewelry shop under the name "Odlew bizuterii" (jewelry casting), at https://www.aejaca.com/shop/jewelry/ , and leads to the same page: a customer asking about casting a ring or a pendant does not have to look in the sTuDiO section. In the sTuDiO shop it keeps its own name, "Odlew z metali szlachetnych". Page: https://www.aejaca.com/shop/service/precious_metal_casting/ .

**Engraving on jewelry and on castings, new price list since 2026-09-03:** two options, both laser-cut after polishing, so on a casting they need the jewellery finish level. "Inicjaly lub krotki napis" (initials or a short line, up to 30 characters) costs 50 PLN; "Grafika lub dluzszy tekst" (artwork or a longer text, up to 120 characters) costs 100 PLN. THE THRESHOLD: when the price of ONE PIECE without the engraving exceeds 400 PLN, the initials are included at no charge and the artwork or longer text costs half, so 50 PLN. The surcharge is per piece, not per order, because every piece is engraved separately; five rings with initials below the threshold cost 5 x 50 PLN. The comparison is against the price of one piece, NOT against the cart total: a cart of two 250 PLN items does not reach the threshold. These are prices the customer pays, with no further margin added. The old "text + pattern, both sides" option is gone: two sides is a packaging idea, and the box has its own separate engraving fields for the lid and its underside. Ordered together with plating, the engraving goes on first and stays visible under the layer.
...
**Question order:** ① what to make (with two upload tiles at the top: "Mam gotowy plik 3D" for a mesh, "Mam gotowy plik wektorowy" for a vector drawing) → ② how big → ③ material → ④ quality → ⑤ how many. The what-to-make question stays active even after a file is uploaded, because it decides material and technology (a technical part gets a different filament than a keychain). As of 2026-08-19, once a file is uploaded the tile question's label switches from "Co chcesz wykonac?" (what do you want to make?) to "Do czego to sluzy?" (what is it for?), so it stops competing with the file that is already on screen; the tiles still drive material and technology choice.
...
- **Unit detection on upload (2026-08-19):** STL and OBJ carry no unit at all, so we read them as millimetres. If that gives an absurdly small size, tell the customer this is the most likely explanation: the file was probably exported in metres (or cm/inches). Both Quick Quote and the advanced 3D print calculator show a panel with the plausible readings (metres, centimetres, inches) and rescale the model to the chosen one in one click, no manual re-export needed.
...
**Uploaded model survives the FDM/MSLA switch in advanced mode (since 2026-08-21):** the advanced 3D print calculator keeps one uploaded file and one size for both technologies. A customer who uploads a model under FDM and switches to MSLA to compare keeps the file and its dimensions, and switching back keeps them too; there is no second upload. The build volume checked follows the machine, so a model that fits the Bambu Lab H2D (300 x 320 x 325 mm) but not the Elegoo Saturn 4 Ultra (218 x 123 x 250 mm) is flagged as soon as the customer switches to resin.

**Engraving is priced from the head's travel, not from the bounding box (since 2026-08-21):** when a vector drawing is uploaded, the engraving price no longer comes from the rectangle around the artwork. The drawing is measured: we rasterise it and sum the SPAN OF MARKS PER ROW, because that is how a raster head works. It skips an empty row entirely and, on a row with marks, travels from the first mark to the last plus overscan. So a thin design on a large sheet is now far cheaper than it was, while a photo engraving covering the whole area costs the same as before. A rectangular frame still counts as full coverage, because the head crosses the whole width on every row between its sides. Coverage scales the MACHINE TIME only: the board and its preparation stay at the full rectangle, because we buy the sheet whole. Quotes made from the size ranges (no file) are unchanged, since the customer states the engraved area itself. The measured coverage is shown in the breakdown and in the item summary. This is what closed the gap the owner asked about: the same file cut cost 18 PLN and engraved 157 PLN, because cutting counted the path the machine travels and engraving counted the whole page.

**Material is a priced item when it comes from our stock (since 2026-08-20):** the quote adds the material only when AEJaCA supplies it. The formula is the area of the piece times 1.15 (waste between parts and at the sheet edges) times a per-square-metre rate held in a database table, and it appears as its own "Materiał / szt." line in the breakdown. If the customer sends their own board or their own object, no material is charged. Rates are the real market ones (plywood 3 mm 24 PLN/m2, acrylic 5 mm 265, solid oak 115, stainless steel 300, brass 750); materials that are not bought by the metre carry a per-piece price instead (glass 12 PLN, slate 15 PLN) and that price wins over the per-square-metre one. Silver and gold are settled by weight, so they are quoted individually and the breakdown says so rather than hiding the line. The table holds our PURCHASE cost; the customer price adds a 50% markup covering purchase, transport, storage, capital tied up in sheets and offcuts that cannot be reused. An unknown material falls back to 140 PLN/m2 of cost, the median of the table. Rates are edited in the admin panel, so if someone asks what a material costs, the answer is that it is priced per square metre of the sheet actually used, confirmed when the job starts. Before this change cutting always charged for material even when the customer supplied it, and engraving never charged even when the board was ours.

**Material tile narrows the stock list, and the machine decides cut vs engrave (since 2026-08-20):** in Quick Quote the material tile ("Drewno", "Metal", "Szklo / kamien / inne") now filters the "z naszego magazynu" list below it, so wood shows wood, metal shows metal. The fibre list opens with silver and gold. Solid wood joined the offer: 10 mm can be cut, thicker than 10 mm is engraving only. What can be cut and what can only be engraved comes from the price list itself, one pair of tables shared by Quick Quote, Advanced mode and the shop, so all three agree by construction. The CO2 laser (xTool P2, 55 W) engraves glass, stone and thick solid wood but CANNOT cut them; it does not process metal at all. Metal marking is the fibre laser, and once metal is chosen the "cut or engrave" question STAYS on screen with cutting greyed out and the reason given ("Metalu nie tniemy: laser swiatlowodowy znakuje powierzchnie"), because a section that vanishes teaches nothing. Engraving asks for the KIND of wood (solid wood, plywood, other wood-based boards) since the beam only reaches the surface; cutting asks for thickness instead (plywood 2/3/5-6 mm, HDF board up to 6 mm, solid wood up to 10 mm). If someone asks whether we can cut glass, stone or metal: no, we engrave or mark them.

**Vector preview zooms and crops to the drawing (since 2026-08-20):** an SVG preview used to be drawn from the whole canvas, so a small mark exported on an A4 sheet showed as a nearly invisible line on a large empty rectangle and the customer could not tell whether the right file had been uploaded. The preview now crops to the drawing itself (about 13x magnification for a 20 x 20 mm mark on A4), and the customer can zoom with the mouse wheel, drag to pan, and press "Dopasuj" to fit again. It works the same in Quick Quote and in Advanced mode. DXF still has no picture preview, only the measured geometry, because the format carries coordinates rather than an image.

**Quick Quote names the plastic, and says where to change it (since 2026-08-19):** each printing card shows the material we picked for that job, "Filament: PLA" or the resin by name, instead of the generic word "filament". Right under the cards there is a note saying we chose it ourselves and that Advanced mode lets the customer pick from 21 filaments (PETG, TPU 95A, ABS among them) or 13 resins, plus infill, number of colours and precision; for laser work the same note lists material and thickness, detail level, complexity and work area. The note carries a button that switches to Advanced mode ON THE MATCHING TAB (engraving opens the CO2 calculator, not 3D printing), and for printing also a link to the shop. **The uploaded file travels with the customer**: switching to Advanced mode carries the model or drawing over together with the size set on the slider, and clicking through to the shop hands it to the service card the customer opens there (once, within fifteen minutes, and only to a card that accepts that kind of file). So nobody has to upload the same file twice; a hard page reload is the one case that loses it. So: if someone asks whether they can have PETG, ABS, TPU, a specific resin or a specific infill, the answer is yes, in the Advanced mode of the estimator at aejaca.com/studio/ (or in the shop for ready-made items), not "we only print in PLA".
...
- **On AEJaCA material**: the client PICKS the material from a list, and the pick changes the price. CO2 engraving: wood, plywood, acrylic, glass, leather, paper, fabric, rubber, stone. CO2 cutting, with thickness because it drives the price: plywood 2 mm, 3 mm and 5-6 mm, HDF board up to 6 mm, solid wood up to 10 mm, acrylic 3/5/8 mm, leather 1-2 mm and 3-4 mm, paper/cardboard, fabric/felt, rubber 2-3 mm. Fiber: stainless steel, aluminium, anodised aluminium, brass, copper, titanium. Silver and gold are engraved but NOT supplied from our stock, so they are not on this list. "Other material" is always available and opens a free-text field. The material IS part of the amount: every laser service (CO2 cutting, CO2 engraving and fiber marking alike) adds a "Material / pc" line computed from our stock table, and adds it only when the material is ours. Availability is confirmed at the start of the job. Nothing is shipped to us.
...
**Metals:** Ag 800 · Ag 925 · Au 9k · Au 14k · Au 18k (no 24k: pure gold is not cast)
**Finishing:** Raw casting · Cut and cleaned (+70 PLN) · Jewellery finish (+160 PLN)
**Automatic binding price ONLY for:** 3D model route + AEJaCA metal + model fitting a 63 mm circle and 62 mm of height after the chosen uniform scale. Volume is measured server-side, mass follows from the alloy density, and a 12% process reserve covers metal left in the sprues and lost to oxidation. Pattern preparation adds 120 PLN.
...
**When a customer says the casting has no price:** the usual cause is the flask limit. BOTH modes now say so on screen and offer one click that scales the model to the largest size that fits, exactly as the printer plate and the laser bed already do. In the quick quote the button sits in an amber panel under the size slider and names the target in centimetres; in advanced mode it is the "Dostosuj mozliwosci techniczne" button under the scale slider. Keeping the original size stays a valid choice, it just moves the order to an individual review. The other causes of a missing price are: no 3D file at all, a physical pattern or idea-only route, customer-supplied metal, and a run above ten pieces.
...
**Model file intake, six questions on the 3D model route (since 2026-09-13):** once the customer picks the 3D model route, six questions run before pricing and none of them has a silent default.
1. File state (\`STANY_MODELU\`): "Finished dimensions" (the file is 1:1, no shrinkage and no allowance, we add the rest) or "Already scaled for shrinkage" (the customer names the ALLOY they scaled for; we add only the difference between that alloy's shrinkage and the one actually ordered). **The customer never types their own multiplier**, only the alloy name; the factor always comes from our own table (\`castingAlloys.js\`), so a typo in a text field cannot enter the piece. Whenever a customer asks whether they should scale the file themselves: no, they name the alloy they designed for (or say the file is at finished size) and AEJaCA does the rest.
2. Item category (\`KATEGORIE_WYROBU\`): ring or wedding band, bangle, pendant/charm, earrings, something else. Only a ring/band and a bangle carry a finger or wrist hole and trigger question 3; a pendant's hole is for a jump ring and is never ground to size, so asking its diameter would be a question about nothing.
3. Hole diameter in the FINISHED piece, ring/band and bangle only. AEJaCA adds a 0.15 mm allowance on the diameter before applying shrinkage, because the hole is always ground after casting (the cast surface is too rough for skin) and grinding ENLARGES a hole, so the model has to start smaller than the target. The customer states whether that allowance is already built into their file, so it is never added twice.
4. Whether the outer surface is decorated (texture, hammering, engraving, relief). A decorated surface gets ZERO allowance, ever, because grinding would erase the pattern the customer ordered. A plain surface only gets a 0.10 mm polish allowance when the finishing level chosen is polished or jewellery finish.
...
- **Blokuja TYLKO trzy rzeczy**, bo tylko przy nich wydruk nie powstanie: model nie miesci sie na stole nawet po obrocie, plik zawiera powierzchnie zamiast zamknietej bryly, albo PRZEWAZAJACA CZESC modelu jest cienciejsza niz jedna sciezka przy wybranych ustawieniach. Wtedy przycisk zmienia napis na "Potwierdz uwagi do modelu".
- **Bramka jest dwuetapowa od 2026-08-19.** Zanim poprosi o potwierdzenie, NAJPIERW pokazuje jak poprawic plik: wymagana grubosc w mm, o ile powiekszyc model, konkretne narzedzia (Blender, Meshmixer) i alternatywe w postaci druku z zywicy. Dopiero gdy klient swiadomie wybierze "chce zamowic ten plik bez zmian", pokazuje sie ryzyko i prosba o potwierdzenie. Przycisk odblokowuje sie po zaznaczeniu potwierdzenia.
...
- Potwierdzenie zapisuje sie przy zamowieniu razem z cala droga (ze instrukcje poprawy pokazano najpierw i ze klient mimo to poszedl dalej) i **wraca w mailu potwierdzajacym**, w jezyku zamowienia.
...
- **Ważność 7 dni.** Robocizna jest w tym okresie wiążąca.
...
bloki dla płatności z Polski (złote, BLIK i szybki przelew, rezerwacja 15 minut od ostatniej
próby zapłaty) i z zagranicy
...
- **Zamówienie da się zamknąć bez realizacji na każdym etapie, także po zapłacie** (od 2026-08-31), a to, ile pieniędzy wraca, zależy od powodu. Cztery drogi: odstąpienie konsumenta w 14 dni (towar z półki, wraca wszystko razem z najtańszą wysyłką), odstąpienie dlatego, że AEJaCA nie dowiozła (zwrot wszystkich kwot w 14 dni), anulowanie decyzją AEJaCA (zwrot pełny) i rezygnacja klienta z rzeczy robionej NA ZAMÓWIENIE. Przy tej ostatniej prawa odstąpienia nie ma (regulamin par. 11, art. 38 pkt 3 ustawy o prawach konsumenta) i mówimy o tym przed zamówieniem, więc kwota zwrotu jest wtedy decyzją AEJaCA i bywa częściowa albo żadna. NIE obiecuj klientowi konkretnej kwoty ani pełnego zwrotu przy rzeczy na zamówienie: powiedz, że decyduje powód i stan prac, i poproś o kontakt na contact@aejaca.com z numerem sprawy. Zwrot idzie na rachunek, z którego przyszła płatność, w ciągu 14 dni od decyzji.
...
- Platnosc: BLIK albo szybki przelew online, przez Autopay, w PLN. Dla klienta czytajacego strone po angielsku lub niemiecku ceny sa w EUR (kurs NBP + 8% na roznice kursowe) i platne przelewem SEPA na nasze konto w euro; numer rachunku pokazuje sie po zlozeniu zamowienia i przychodzi mailem, kwota i rezerwacja towaru obowiazuja 3 dni robocze, a czwartego dnia roboczego bez wplaty rezerwacja spada i towar wraca do sprzedazy, wplyw potwierdzamy recznie, a termin realizacji liczy sie od zaksiegowania: liczymy pelne dni od dnia nastepnego, dzien wplaty sie nie liczy.
