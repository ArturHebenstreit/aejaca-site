...
// ------------------------------------------------------------
// Liczba bryl i otwor osiowy
// ------------------------------------------------------------
// Oba pomiary powstaly dla ODLEWU z pliku klienta, nie dla druku, ale mieszkaja
// tutaj, bo to jest jedyne miejsce w serwisie, ktore czyta topologie siatki.
// Druga kopia tych petli w katalogu odlewniczym rozjechalaby sie z ta przy
// pierwszej poprawce sklejania wierzcholkow.

/**
 * Liczba rozlacznych bryl w pliku.
 *
 * Dwie bryly, ktore sie PRZENIKAJA, ale nie zostaly zlozone `BooleanUnion`,
 * nie maja wspolnych wierzcholkow, wiec wychodza tu jako dwie. To jest cel
 * tego pomiaru: wnetrze takiej pary jest niezdefiniowane, slicer zgaduje, a
 * odlew utrwala zgadniete. Liczymy przez zbiory rozlaczne na wierzcholkach
 * sklejonych tym samym kwantem, co reszta topologii.
 */
export function analyzeSolids(triangles) {
  if (!triangles.length) return { solids: 0, largestShare: 0 };
  ...
  for (let i = 0; i < triangles.length; i++) {
    const t = triangles[i];
    tri[i * 3] = idOf(t[0]); tri[i * 3 + 1] = idOf(t[1]); tri[i * 3 + 2] = idOf(t[2]);
  }

  const parent = new Int32Array(vertexId.size);
  for (let i = 0; i < parent.length; i++) parent[i] = i;
  const find = (x) => {
    let r = x;
    while (parent[r] !== r) r = parent[r];
    // Skracanie sciezki w osobnej petli, bo bez niego przy dlugim lancuchu
    // scalen ta funkcja robi sie liniowa i caly pomiar kwadratowy.
  ...
  for (let i = 0; i < triangles.length; i++) {
    union(tri[i * 3], tri[i * 3 + 1]);
    union(tri[i * 3 + 1], tri[i * 3 + 2]);
  }

  // Udzial najwiekszej bryly liczymy POLEM, a nie liczba trojkatow: siatka
  // bywa gesta tam, gdzie jest krzywizna, wiec liczba trojkatow nie mowi nic
  // o tym, czy druga bryla to polowa wyrobu, czy zabakany punkt.
  const pole = new Map();
  let suma = 0;
  for (let i = 0; i < triangles.length; i++) {
    ...
    const r = find(tri[i * 3]);
    pole.set(r, (pole.get(r) || 0) + ar);
    suma += ar;
  }

  const najwieksza = Math.max(...pole.values());
  return { solids: pole.size, largestShare: suma > 0 ? najwieksza / suma : 0 };
}

/**
 * Srednica otworu, ktory przechodzi przez model na wylot.
 *
 * Sluzy jednej rzeczy: sprawdzeniu, czy otwor w pliku zgadza sie z rozmiarem
 * palca, ktory klient podal w formularzu. Roztoczenie otworu zabiera grubosc
 * szyny, wiec roznicy nie wolno poprawic po cichu.
 *
 * Jak to dziala: srodek bryly przy obraczce lezy W POWIETRZU, w otworze.
 * Puszczamy z niego wiazke promieni w plaszczyznie prostopadlej do kazdej
 * z trzech osi. Wzdluz osi otworu wszystkie promienie trafiaja w metal, bo
 * obwod jest zamkniety. Wzdluz pozostalych dwoch czesc promieni ucieka przez
 * otwor i nie trafia w nic. Os otworu rozpoznaje sie wiec sama, bez pytania
 * klienta, w ktorej osi zamodelowal.
 *
 * Zwraca `null`, gdy zadna os nie daje pelnego pierscienia trafien: model nie
 * jest obraczka albo srodek wypada w metalu. Wtedy nie mamy czego porownac
 * i mowimy to wprost, zamiast zgadywac.
 */
export function analyzeAxialHole(triangles, { rays = 180 } = {}) {
  if (triangles.length < 4) return null;
  const b = boundsOf(triangles);
  const grid = buildGrid(triangles);
  const maxDist = Math.hypot(b.size[0], b.size[1], b.size[2]);

  // Wiazka promieni w plaszczyznie prostopadlej do osi `os`, z punktu `p`.
  // Zwraca najmniejsza i najwieksza odleglosc oraz kierunek najblizszego
  // trafienia. `null`, gdy ktorykolwiek promien uciekl w powietrze.
  //
  // NIE WYMAGAMY, ZEBY TRAFILY WSZYSTKIE. Pierscionek z pave ma gniazda
  // przewiercone na wylot, wiec promien wycelowany w gniazdo wychodzi bokiem
  // i pierwsza wersja tego pomiaru odmawiala odpowiedzi na kazdym pave.
  // Przy zlej osi ucieka polowa wiazki, wiec prog jednej piatej nadal
  // rozdziela os otworu od pozostalych dwoch.
  const wiazka = (os, p, ile) => {
    const u = (os + 1) % 3, v = (os + 2) % 3;
    let min = Infinity, kierunek = null, trafien = 0;
    const odleglosci = [];
    for (let i = 0; i < ile; i++) {
      const kat = (2 * Math.PI * i) / ile;
      const dir = [0, 0, 0];
      dir[u] = Math.cos(kat);
      dir[v] = Math.sin(kat);
      const t = firstHit(grid, triangles, p, dir, maxDist);
      if (t === null) continue;
      trafien++;
      odleglosci.push(t);
      if (t < min) { min = t; kierunek = dir; }
    }
    if (trafien / ile < 0.8 || !kierunek) return null;
    // Odniesieniem jest trzeci kwartyl, a nie maksimum: promien, ktory wpadl
    // w gniazdo, trafia dopiero w przeciwlegla sciane i samo maksimum
    // zrobiloby z rownego pierscienia owal.
    odleglosci.sort((a, b) => a - b);
    const p75 = odleglosci[Math.min(odleglosci.length - 1, Math.floor(odleglosci.length * 0.75))];
    return { min, max: p75, kierunek };
  };

  // PUNKT MUSI LEZEC W POWIETRZU, nie w metalu. Bez tego sprawdzenia lita
  // kostka odpowiadala "otwor 10 mm", bo promienie ze srodka trafiaja w
  // sciany od wewnatrz dokladnie tak samo, jak w otworze obraczki. Parzysta
  // liczba przeciec z powierzchnia oznacza punkt na zewnatrz bryly.
  const wMetalu = (punkt) => {
    // Kierunek lekko skosny, zeby promien nie biegl wzdluz sciany ani przez
    // krawedz siatki, gdzie zliczanie przeciec jest niejednoznaczne.
    const dir = [0.5773, 0.5774, 0.5775];
    let przeciec = 0;
    for (const t of triangles) {
      if (rayTriangle(punkt, dir, t[0], t[1], t[2]) !== null) przeciec++;
    }
    return przeciec % 2 === 1;
  };
  ...
  for (let os = 0; os < 3; os++) {
    // SRODEK PROSTOKATA OTACZAJACEGO NIE JEST SRODKIEM OTWORU. Soliter ma nad
    // szyna korone, wiec prostokat rozciaga sie w gore i jego srodek wypada
    // dwa milimetry nad osia palca: zmierzona srednica wychodzila wtedy 13 mm
    // zamiast 17,2 mm. Szukamy wiec NAJWIEKSZEGO OKREGU WPISANEGO: zaczynamy
    // od srodka prostokata i w kazdym kroku odsuwamy punkt od najblizszej
    // sciany, dopoki przyrost promienia jest zauwazalny.
    let p = [
      (b.min[0] + b.max[0]) / 2,
      (b.min[1] + b.max[1]) / 2,
      (b.min[2] + b.max[2]) / 2,
    ];
    let stan = wiazka(os, p, 32);
    if (!stan) continue;
    for (let krok = 0; krok < 24; krok++) {
      const luz = (stan.max - stan.min) / 2;
      if (luz < 1e-3) break;
      const dalej = [
        p[0] - stan.kierunek[0] * luz * 0.5,
        p[1] - stan.kierunek[1] * luz * 0.5,
        p[2] - stan.kierunek[2] * luz * 0.5,
      ];
      const nowy = wiazka(os, dalej, 32);
      // Krok, ktory pogarsza wynik albo wyprowadza punkt poza otwor, odrzucamy
      // i konczymy: dalsze bladzenie nie ma juz gdzie poprawic.
      if (!nowy || nowy.min <= stan.min + 1e-4) break;
      p = dalej;
      stan = nowy;
    }

    if (wMetalu(p)) continue;
    const pomiar = wiazka(os, p, rays);
    if (!pomiar) continue;
    // Otwor na palec ma promien prawie staly. Rozrzut ponad polowe promienia
    // znaczy, ze to nie jest otwor na palec, tylko na przyklad wnetrze
    // pudelka, i porownywanie tego z rozmiarem palca nie mialoby sensu.
    const owal = pomiar.max > 0 ? (pomiar.max - pomiar.min) / pomiar.max : 1;
    if (owal > 0.5) continue;
    if (!najlepsza || pomiar.min > najlepsza.promien) {
      najlepsza = { os, promien: pomiar.min, owal };
    }
  }

  if (!najlepsza) return null;
  return {
    axis: ["x", "y", "z"][najlepsza.os],
    diameterMm: najlepsza.promien * 2,
    // Owalizacja przydaje sie przy modelu owalnym z zalozenia (komfortowa
    // obraczka), gdzie sama srednica nie opisuje pasowania.
    ovality: najlepsza.owal,
  };
}
