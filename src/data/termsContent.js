    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          "Przy odlewie z metalu powierzonego przez Klienta wiążąca Wycena jest przedstawiana po zważeniu, udokumentowaniu i weryfikacji materiału. Klient zapewnia uzgodniony zapas technologiczny. Typowy ubytek dla czystego, jednorodnego stopu wynosi 5 do 8%, a dla materiału mieszanego lub o niepewnym składzie może wynieść 10 do 15%. Sprzedawca może odmówić przetopu materiału platerowanego, wypełnianego albo nienadającego się do bezpiecznego procesu.",
      ...
      },
      {
        ...
        items: [
          ...
          "Przy odlewie z modelu Klienta skurcz metalu, naddatek na obróbkę i układ wlewowy dodaje Sprzedawca. Klient wskazuje przed złożeniem zamówienia, czy przesłany plik jest w wymiarach wyrobu gotowego, czy został już powiększony o skurcz, a w tym drugim przypadku podaje stop, dla którego skalowanie wykonano. Wskazanie to jest podstawą przeliczenia wymiaru, a wynik tego przeliczenia Serwis pokazuje przed potwierdzeniem zamówienia.",
          "Serwis bada przesłany model do odlewu także pod względem szczelności bryły, liczby brył, najcieńszej ścianki oraz zgodności otworu z wymiarem podanym przez Klienta. Jeżeli badanie wykaże właściwość, o której Serwis ostrzega, a Klient potwierdzi jej przyjęcie i poleci wykonanie, ponowne wykonanie odlewu, którego niepowodzenie wynika z tej właściwości, następuje na koszt Klienta. Jeżeli Serwis nie zgłosił ostrzeżenia, a odlew nie powiódł się z przyczyn leżących w geometrii modelu, ponowne wykonanie następuje na koszt Sprzedawcy. Treść ostrzeżeń oraz potwierdzenie Klienta Sprzedawca utrwala przy zamówieniu.",
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          "For casting from metal supplied by the Customer, a binding Quotation is issued after the material has been weighed, documented and verified. The Customer supplies the agreed technological reserve. Typical loss for a clean single alloy is 5 to 8%; mixed material or uncertain composition may incur 10 to 15%. The Seller may refuse plated, filled or otherwise unsuitable material.",
      ...
      },
      {
        ...
        items: [
          ...
          "For a casting made from a Customer model, the Seller adds the metal shrinkage, the machining allowance and the sprue layout. Before placing the order the Customer states whether the uploaded file carries the dimensions of the finished item or has already been scaled for shrinkage, and in the latter case names the alloy the scaling was done for. That statement is the basis for the dimension calculation, and the Service shows the result of that calculation before the order is confirmed.",
          "The Service also examines a model submitted for casting for a watertight solid, the number of solids, the thinnest wall and the agreement of the hole with the dimension given by the Customer. Where the examination reveals a characteristic the Service warns about, and the Customer confirms having received that warning and instructs production, a repeat casting whose failure follows from that characteristic is made at the Customer expense. Where the Service raised no warning and the casting failed for reasons lying in the geometry of the model, the repeat casting is made at the Seller expense. The Seller records the content of the warnings and the Customer confirmation with the order.",
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          "Beim Guss aus vom Kunden beigestelltem Metall wird ein verbindliches Angebot erst nach Wiegen, Dokumentation und Prüfung des Materials erstellt. Der Kunde stellt die vereinbarte technologische Reserve bereit. Bei einer sauberen, einheitlichen Legierung beträgt der typische Verlust 5 bis 8%, bei gemischtem oder unsicherem Material 10 bis 15%. Der Verkäufer kann plattiertes, gefülltes oder anderweitig ungeeignetes Material ablehnen.",
      ...
      },
      {
        ...
        items: [
          ...
          "Beim Guss aus einem Kundenmodell ergänzt der Verkäufer den Metallschwund, das Bearbeitungsaufmaß und das Angusssystem. Vor der Bestellung gibt der Kunde an, ob die hochgeladene Datei die Maße des fertigen Erzeugnisses trägt oder bereits auf den Schwund skaliert wurde, und nennt im zweiten Fall die Legierung, für die skaliert wurde. Diese Angabe ist die Grundlage der Maßberechnung, deren Ergebnis der Service vor der Bestätigung der Bestellung anzeigt.",
          "Der Service prüft ein zum Guss eingereichtes Modell zusätzlich auf einen geschlossenen Körper, die Anzahl der Körper, die dünnste Wand und die Übereinstimmung der Bohrung mit dem vom Kunden angegebenen Maß. Ergibt die Prüfung eine Eigenschaft, vor der der Service warnt, und bestätigt der Kunde deren Kenntnisnahme und beauftragt die Ausführung, so erfolgt ein erneuter Guss, dessen Misslingen auf dieser Eigenschaft beruht, auf Kosten des Kunden. Hat der Service keine Warnung ausgegeben und ist der Guss aus Gründen der Modellgeometrie misslungen, erfolgt der erneute Guss auf Kosten des Verkäufers. Der Verkäufer hält den Inhalt der Warnungen und die Bestätigung des Kunden bei der Bestellung fest.",
