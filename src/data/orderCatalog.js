...
// Sciezka wzgledem WLASNEGO katalogu, a nie przez `../data/`: ten plik jest
// kopiowany do `chat-api/pricing/` razem z rdzeniem cenowym (`npm run
// sync:pricing`), zeby serwer pocztowy opisywal parametry tym samym slownikiem
// co koszyk. Zapis `../data/resins.js` szukalby tam katalogu, ktorego nie ma.
...
/**
 * Usluga, do ktorej nalezy kalkulator.
 *
 * `order_items` nie ma kolumny z identyfikatorem uslugi, tylko nazwe
 * kalkulatora. Kazdy kalkulator nalezy do dokladnie jednej uslugi, wiec
 * po zaplacie da sie odtworzyc karte pytan i opisac wybory klienta slowami,
 * takze dla zamowien zlozonych wczesniej. Mapa stoi TUTAJ, a nie osobno
 * po stronie serwera i osobno po stronie strony: dwie kopie rozjechalyby sie
 * przy dodaniu uslugi, a objawem bylaby pozycja bez opisu.
 */
export function uslugaKalkulatora(calculator) {
  return SERVICES.find((s) => s.calculator === calculator)?.id || null;
}
