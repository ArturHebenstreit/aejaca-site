...
import { CASTING_ENVELOPE_LABEL, CASTING_FLASK_MM, CASTING_METALS, PRECIOUS_METAL_CASTING_BUILD } from "../pricing/preciousMetalCasting.js";

/** Proby w postaci "Ag 925/800, Au 9k/14k/18k", liczone z listy kruszcow. */
const SPIS_KRUSZCOW = (() => {
  const grupa = (przedrostek, skrot) => `${skrot} ${CASTING_METALS
    .filter((m) => m.id.startsWith(przedrostek))
    .map((m) => m.label.pl.replace(/^(Srebro|Złoto)\s*/, "").replace(/\s*\(.*\)$/, ""))
    .join("/")}`;
  return `${grupa("silver", "Ag")}, ${grupa("gold", "Au")}`;
})();
import { ENGRAVING_OPTIONS, ENGRAVING_FREE_ABOVE_PLN } from "../pricing/jewelryConfig.js";
import { CONFIG } from "../pricing/config.js";

// KWOTY GRAWERU IDA Z CENNIKA, nie z recznie przepisanej liczby. Karta uslugi
// podaje je w dwoch miejscach (opis i tabela danych), a tabela cen jest trzecim:
// przepisane recznie rozjechalyby sie przy pierwszej zmianie i klient
// przeczytalby na jednej stronie dwie ceny tego samego graweru.
const GRAWER_PLN = Object.fromEntries(ENGRAVING_OPTIONS.map((o) => [o.id, o.pricePLN]));
const wEuro = (pln) => Math.round(pln / CONFIG.EUR_PLN_RATE);
const PROG_GRAWERU_EUR = wEuro(ENGRAVING_FREE_ABOVE_PLN);
...
export const SERVICES_FULL = [
  ...
  },
  {
    ...
    description: L(
      "Odlew żywiczny pozwala zamknąć coś na stałe: kwiaty ze ślubnego bukietu, pamiątkę, element elektroniczny albo źródło światła. Powstaje przedmiot, którego nie da się wykonać żadną inną techniką, bo zatopienie jest widoczne przez materiał.\n\nPracujemy na żywicy UV, która utwardza się w minuty i nadaje się do małych form, oraz na epoksydowej, która wymaga doby, ale za to wolniej wiąże, więc powietrze ma czas uciec z bryły, a materiał nie grzeje się nadmiernie przy większej objętości.\n\nFormę bierzemy z naszego katalogu, robimy nową z przedmiotu, który przyślesz, z Twojego pliku 3D albo z projektu, który narysujemy. Możesz też przysłać własną. Nowa forma to osobny etap: wydruk wzorca, wypolerowanie go do lustra i wylanie silikonu w dwóch odsłonach, dlatego takie zlecenie trwa tygodnie, a nie dni.\n\nPrzy żywicy bezbarwnej mówimy wprost: odgazowujemy mieszankę i lejemy warstwami, ale nie mamy garnka ciśnieniowego, więc nie obiecujemy odlewu całkiem bez pęcherzyków. Przy zatopieniach z metalu i przy ostrych krawędziach pojedyncze potrafią zostać. Wykończenie ustalamy osobno: od surowego, przez szlifowane, po polerowane na lustro.",
      "Resin casting lets you seal something permanently: flowers from a wedding bouquet, a keepsake, an electronic component or a light source. The result is an object no other technique can produce, because the inclusion stays visible through the material.\n\nWe work with UV resin, which cures in minutes and suits small molds, and with epoxy, which needs a day but cures slowly, so air has time to escape the block and the material does not overheat at larger volumes.\n\nWe take the mold from our catalogue, make a new one from an object you send, from your 3D file, or from a design we draw. You can also send your own. A new mold is a stage of its own: printing the pattern, polishing it to a mirror and pouring the silicone in two halves, which is why such an order takes weeks rather than days.\n\nWith clear resin we say it plainly: we degas the mix and pour in layers, but we have no pressure pot, so we do not promise a cast entirely free of bubbles. Around metal inclusions and sharp edges the odd one can remain. Finishing is agreed separately: from raw, through sanded, to mirror polished.",
      "Harzguss erlaubt es, etwas dauerhaft einzuschließen: Blumen aus dem Hochzeitsstrauß, ein Andenken, ein elektronisches Bauteil oder eine Lichtquelle. Es entsteht ein Objekt, das keine andere Technik hervorbringt, denn der Einschluss bleibt durch das Material sichtbar.\n\nWir arbeiten mit UV-Harz, das in Minuten aushärtet und für kleine Formen geeignet ist, sowie mit Epoxid, das einen Tag braucht, dafür langsam aushärtet, sodass Luft aus dem Block entweichen kann und das Material bei größeren Volumen nicht überhitzt.\n\nDie Form nehmen wir aus unserem Katalog, fertigen eine neue von einem Objekt, das Sie senden, aus Ihrer 3D-Datei oder nach einem Entwurf, den wir zeichnen. Sie können auch Ihre eigene schicken. Eine neue Form ist ein eigener Schritt: Druck des Musters, Politur auf Hochglanz und Gießen des Silikons in zwei Hälften, deshalb dauert ein solcher Auftrag Wochen und nicht Tage.\n\nBei klarem Harz sagen wir es klar: Wir entgasen die Mischung und gießen in Schichten, doch wir haben keinen Drucktopf, versprechen also keinen völlig blasenfreien Guss. An Metalleinschlüssen und scharfen Kanten kann die eine oder andere bleiben. Das Finish wird separat vereinbart: von roh über geschliffen bis spiegelpoliert."
    ),
    process: [
      ...
      { title: L("Zalewanie warstwami", "Pouring in layers", "Gießen in Schichten"),
        body: L("Odgazowujemy żywicę i lejemy warstwami, żeby zatopienie stanęło tam, gdzie ma. Każda warstwa utwardza się dwie doby.", "We degas the resin and pour in layers so the inclusion sits where it should. Each layer cures for two days.", "Wir entgasen das Harz und gießen in Schichten, damit der Einschluss richtig sitzt. Jede Schicht härtet zwei Tage aus.") },
    ...
    specs: [
      ...
      { label: L("Zatopienia", "Inclusions", "Einschlüsse"), value: L("Pigment, pamiątki, elektronika LED", "Pigment, keepsakes, LED electronics", "Pigment, Andenken, LED-Elektronik") },
      { label: L("Napis w bryle", "Text in the block", "Schrift im Block"), value: L("Grawer na srebrnej blaszce albo druk w żywicy", "Engraved silver plate or printed in resin", "Gravur auf Silberplättchen oder Druck in Harz") },
      { label: L("Formy", "Molds", "Formen"), value: L("Nasza, z Twojego przedmiotu, z pliku 3D, z naszego projektu albo Twoja", "Ours, from your object, from a 3D file, from our design, or yours", "Unsere, von Ihrem Objekt, aus einer 3D-Datei, nach unserem Entwurf oder Ihre") },
      // TERMIN ZALEZY OD FORMY i to jest fizyka, nie ostroznosc: silikon musi
      // zwiazac, zanim cokolwiek wlejemy. Karta podaje `leadTimeDays: 7`, ktore
      // dotyczy formy stojacej u nas na polce; ten wiersz mowi reszte, zeby
      // klient nie czytal siedmiu dni jako obietnicy na nowa forme.
      { label: L("Czas realizacji", "Lead time", "Bearbeitungszeit"), value: L("7-14 dni z gotowej formy, 14-21 dni z nową", "7-14 days from an existing mold, 14-21 days with a new one", "7-14 Tage aus vorhandener Form, 14-21 Tage mit neuer") },
    ...
    bullets: [
      ...
      L("Forma nasza, z Twojego przedmiotu, z pliku albo Twoja", "Mold ours, from your object, from a file, or yours", "Form unsere, von Ihrem Objekt, aus einer Datei oder Ihre"),
      L("Żywica odgazowana, zalewanie warstwami", "Resin degassed, poured in layers", "Harz entgast, in Schichten gegossen"),
    ],
  },
  // ---------------- Bizuteria ----------------
  {
  ...
  },
  {
    ...
    // krotszy opis zostaje zastapiony wlasnym urywkiem strony.
    seoOpis: L(
      `Odlew ze srebra i złota z wzorca, modelu 3D albo samego pomysłu. Kruszec nasz lub powierzony, kolba ${CASTING_FLASK_MM.diameter} mm, wykończenie w pięciu stopniach. Cena z modelu.`,
      `Silver and gold casting from a pattern, a 3D model or just an idea. Our metal or yours, an ${CASTING_FLASK_MM.diameter} mm flask, five levels of finishing. Priced from your model.`,
      `Silber- und Goldguss aus Modell, 3D-Datei oder bloßer Idee. Unser oder Ihr Edelmetall, Küvette ${CASTING_FLASK_MM.diameter} mm, fünf Finish-Stufen. Verbindlicher Preis aus Ihrem Modell.`
    ),
    description: L(
      `Jedna usługa obejmuje trzy punkty startowe. Możesz dostarczyć gotowy wzorzec z wosku lub żywicy odlewniczej, przesłać model 3D do wydruku albo opisać sam pomysł, a AEJaCA sTuDiO przygotuje model, wzorzec i odlew. Każda ścieżka kończy się odlewem ze srebra albo złota w wybranej próbie.\n\nWiążącą cenę podajemy automatycznie wtedy, gdy przesłany model 3D pozwala policzyć objętość, mieści się w ograniczeniach kolby i wybierasz kruszec AEJaCA. Gotowy wzorzec trzeba najpierw obejrzeć, a przedmiot wykonywany od pomysłu najpierw zaprojektować. Te dwa warianty przechodzą do wyceny indywidualnej.\n\nMożemy pracować na kruszcu AEJaCA albo na materiale powierzonym. Materiał powierzony badamy przed przyjęciem. Dla czystego, jednorodnego stopu trzeba zwykle zapewnić 5 do 8 procent zapasu na ubytek technologiczny, a dla stopu mieszanego lub nieznanego 10 do 15 procent. Materiał platerowany lub wypełniany może nie nadawać się do przetopu.\n\nWykończenie jubilerskie to ostatni z pięciu poziomów i obejmuje cztery rzeczy: odcięcie kanałów wlewowych i zlicowanie śladu z powierzchnią, przeszlifowanie całej bryły papierami o rosnącej gradacji, polerowanie do połysku na kole z pastą oraz oględziny przed wydaniem, podczas których sprawdzamy pory, niedolewy i krawędzie. Na tym poziomie domawia się też grawer, wykonany laserem po wypolerowaniu: inicjały albo krótki napis za ${GRAWER_PLN.text} zł, a grafika albo dłuższy tekst za ${GRAWER_PLN.pattern} zł. Powyżej ${ENGRAVING_FREE_ABOVE_PLN} zł ceny sztuki inicjały wchodzą w cenę, a grafika i dłuższy tekst kosztują połowę. Zamówiony razem z powłoką idzie przed nią, więc ślad zostaje widoczny pod warstwą. Na tym poziomie można dobrać powłokę galwaniczną: rod daje biel na srebrze i złocie, złocenie żółte albo różowe zmienia barwę wierzchu. Powłoka kładzie się wyłącznie na wypolerowaną powierzchnię, bo galwanika odwzorowuje to, co pod nią leży. Warstwa z czasem się ściera i odnawia się ją serwisowo.\n\nGdy przysyłasz własny wzorzec, wosk albo wydruk z żywicy odlewniczej, obowiązują dodatkowe warunki i pokazujemy je w konfiguratorze zaraz po wybraniu tej ścieżki. W skrócie: podajesz materiał wzorca, a przy wydruku nazwę i producenta żywicy, bo pod nie dobieramy profil wypalania. Wzorzec ma dotrzeć wypłukany i w pełni doświetlony. Ulega zniszczeniu, bo wypala się z formy. Rozliczamy wykonaną próbę odlewniczą, a nie jej wynik, ale gdy zawinimy my, powtarzamy odlew na własny koszt. Pełne brzmienie w regulaminie, sekcja 8b.\n\nPracownia jubilerska, która chce odlewać u nas regularnie, może umówić się na współpracę stałą. Warunki ustalamy indywidualnie, w zależności od wolumenu: napisz, ile kolb miesięcznie przewidujesz i w jakim kruszcu, a przedstawimy stawki i termin wejścia do kolejki.`,
      `One service supports three starting points. Supply a ready wax or castable-resin pattern, upload a 3D model for us to print, or describe the idea and AEJaCA sTuDiO will design, print and cast it. Each route ends with a silver or gold casting in the selected purity.\n\nWe provide a binding automatic price when an uploaded 3D model gives a measurable volume, fits our flask limits and uses AEJaCA metal. A physical pattern must be inspected first, while an idea must be designed before its mass is known. Those two routes receive an individual quote.\n\nWe can use AEJaCA metal or customer-supplied metal. Supplied metal is assessed before acceptance. A clean single alloy typically needs 5 to 8 percent extra for technological loss; mixed or unknown material needs 10 to 15 percent. Plated or filled material may be unsuitable for remelting.\n\nThe jewellery finish is the last of the five levels and covers four things: cutting the sprues and dressing the stub flush, grinding the whole body through rising grits, polishing to a shine on a wheel with compound, and an inspection before release where we look for porosity, short pours and edges. At this level you can also add an engraving, laser-cut after polishing: initials or a short line for ${wEuro(GRAWER_PLN.text)} EUR, or artwork or a longer text for ${wEuro(GRAWER_PLN.pattern)} EUR. Above ${PROG_GRAWERU_EUR} EUR per piece the initials are included and the artwork costs half. Ordered together with plating, the engraving goes on first and stays visible under the layer. You can add galvanic plating too: rhodium gives a white finish on silver and gold, yellow or rose plating changes the surface colour. Plating goes only on a polished surface, because it reproduces whatever lies beneath it. The layer wears with time and is renewed as a service.\n\nWhen you send your own pattern, wax or a print in castable resin, additional terms apply and the configurator shows them the moment you pick that path. In short: you state the pattern material, and for a print the name and manufacturer of the resin, because the burnout profile is matched to it. The pattern must arrive washed and fully post-cured. It is destroyed, being burned out of the mould. We charge for the casting attempt performed, not for its outcome, but where the fault is ours we repeat the cast at our own cost. Full wording in section 8b of the terms.\n\nA workshop that wants to cast with us regularly can arrange ongoing cooperation. Terms are agreed individually, depending on volume: tell us how many flasks a month you expect and in which alloy, and we will come back with rates and a slot in the queue.`,
      `Eine Leistung unterstützt drei Ausgangspunkte. Sie liefern ein fertiges Wachs- oder Gießharzmodell, laden ein 3D-Modell zum Druck hoch oder beschreiben Ihre Idee, und AEJaCA sTuDiO übernimmt Entwurf, Druck und Guss. Jede Variante endet als Silber- oder Goldguss im gewählten Feingehalt.\n\nEin verbindlicher automatischer Preis ist möglich, wenn das 3D-Modell messbar ist, in die Gussgrenzen passt und AEJaCA-Metall verwendet wird. Ein physisches Modell muss zuerst geprüft, eine Idee zuerst konstruiert werden. Diese Varianten erhalten ein individuelles Angebot.\n\nWir arbeiten mit AEJaCA-Metall oder beigestelltem Metall. Beigestelltes Metall wird vor Annahme geprüft. Bei einer sauberen, einheitlichen Legierung sind gewöhnlich 5 bis 8 Prozent Reserve für technologischen Verlust nötig, bei gemischtem oder unbekanntem Material 10 bis 15 Prozent. Plattiertes oder gefülltes Material kann ungeeignet sein.\n\nDas Juwelierfinish ist die letzte der fünf Stufen und umfasst vier Schritte: Gusskanäle abtrennen und den Ansatz flächenbündig verputzen, den gesamten Körper mit steigender Körnung schleifen, auf der Scheibe mit Paste auf Hochglanz polieren und vor der Ausgabe prüfen, wobei wir auf Poren, Fehlgüsse und Kanten achten. Auf dieser Stufe lässt sich auch eine Gravur bestellen, nach dem Polieren mit dem Laser ausgeführt: Initialen oder eine kurze Zeile für ${wEuro(GRAWER_PLN.text)} EUR, oder Grafik beziehungsweise längerer Text für ${wEuro(GRAWER_PLN.pattern)} EUR. Über ${PROG_GRAWERU_EUR} EUR je Stück sind die Initialen inklusive und die Grafik kostet die Hälfte. Zusammen mit einer Beschichtung gravieren wir zuerst, damit die Gravur unter der Schicht sichtbar bleibt. Ebenso lässt sich eine galvanische Beschichtung wählen: Rhodium ergibt Weiß auf Silber und Gold, Gelb- oder Roségold ändert die Oberflächenfarbe. Die Beschichtung kommt ausschließlich auf eine polierte Oberfläche, da sie abbildet, was darunter liegt. Die Schicht nutzt sich mit der Zeit ab und wird im Service erneuert.\n\nWenn Sie ein eigenes Modell senden, Wachs oder einen Druck aus Gießharz, gelten zusätzliche Bedingungen; der Konfigurator zeigt sie unmittelbar nach Wahl dieses Wegs. Kurz gefasst: Sie nennen das Material des Modells, bei einem Druck Name und Hersteller des Harzes, denn danach richtet sich das Ausbrennprofil. Das Modell muss gewaschen und vollständig nachgehärtet ankommen. Es wird zerstört, da es aus der Form ausgebrannt wird. Wir rechnen den durchgeführten Gussversuch ab, nicht dessen Ergebnis, doch liegt der Fehler bei uns, wiederholen wir den Guss auf eigene Kosten. Vollständiger Wortlaut in Abschnitt 8b der AGB.\n\nEine Werkstatt, die regelmäßig bei uns gießen möchte, kann eine dauerhafte Zusammenarbeit vereinbaren. Die Bedingungen stimmen wir individuell ab, abhängig vom Volumen: Sagen Sie uns, wie viele Küvetten pro Monat Sie erwarten und in welcher Legierung, und wir nennen Ihnen Sätze und einen Platz in der Warteschlange.`
    ),
    process: [
      ...
      // KROK 3 OPISUJE TRZY DROGI, NIE JEDNA. Mowil "wzorzec drukujemy",
      // takze przy wybranym wariancie "gotowy wzorzec", w ktorym wzorzec
      // przysyla klient i nie drukujemy niczego.
      { title: L("Wzorzec i odlew", "Pattern and cast", "Modell und Guss"), body: L("Twój wzorzec formujemy od razu, model 3D najpierw drukujemy w żywicy odlewniczej 16K. Potem wypalanie i odlew próżniowy.", "Your own pattern is invested straight away, a 3D model is first printed in 16K castable resin. Then burnout and vacuum casting.", "Ihr eigenes Modell wird direkt eingebettet, eine 3D-Datei zuerst in 16K-Gießharz gedruckt. Danach Ausbrennen und Vakuumguss.") },
    ...
    specs: [
      ...
      // Spis prob idzie z listy kruszcow, a nie z przepisanej kopii: kopia
      // obiecywala Au 999 jeszcze po zdjeciu go z oferty (2026-09-10).
      { label: L("Kruszce", "Metals", "Metalle"), value: L(SPIS_KRUSZCOW, SPIS_KRUSZCOW, SPIS_KRUSZCOW) },
      { label: L("Kolba odlewnicza", "Casting flask", "Gussküvette"), value: `\u00d8${CASTING_FLASK_MM.diameter} x ${CASTING_FLASK_MM.depth} mm` },
      { label: L("Automatyczny limit modelu", "Automatic model limit", "Automatische Modellgrenze"), value: L(`${CASTING_ENVELOPE_LABEL}, liczy się przekątna podstawy, nie pojedyncza oś`, `${CASTING_ENVELOPE_LABEL}, the footprint diagonal counts, not a single axis`, `${CASTING_ENVELOPE_LABEL}, es zählt die Diagonale der Grundfläche, nicht eine einzelne Achse`) },
      { label: L("Zakres wykończenia", "Finishing levels", "Finish-Stufen"), value: L("Pięć poziomów, od surowego odlewu po wykończenie jubilerskie. Przy kruszcu AEJaCA bez poziomu z kanałami wlewowymi", "Five levels, from as-cast to jewellery finish. With AEJaCA metal the as-cast level with sprues is not offered", "Fünf Stufen, vom Rohguss bis zum Juwelierfinish. Bei AEJaCA-Metall entfällt die Stufe mit Gusskanälen") },
      { label: L("Powłoka galwaniczna", "Galvanic plating", "Galvanische Beschichtung"), value: L("Rod, złocenie żółte, złocenie różowe. Tylko przy wykończeniu jubilerskim", "Rhodium, yellow gold, rose gold. Only with the jewellery finish", "Rhodium, Gelbgold, Roségold. Nur beim Juwelierfinish") },
      { label: L("Grawer", "Engraving", "Gravur"), value: L(
        `Inicjały ${GRAWER_PLN.text} zł, grafika lub dłuższy tekst ${GRAWER_PLN.pattern} zł. Powyżej ${ENGRAVING_FREE_ABOVE_PLN} zł za sztukę inicjały gratis, grafika za połowę`,
        `Initials ${wEuro(GRAWER_PLN.text)} EUR, artwork or a longer text ${wEuro(GRAWER_PLN.pattern)} EUR. Above ${PROG_GRAWERU_EUR} EUR per piece the initials are included and the artwork costs half`,
        `Initialen ${wEuro(GRAWER_PLN.text)} EUR, Grafik oder längerer Text ${wEuro(GRAWER_PLN.pattern)} EUR. Über ${PROG_GRAWERU_EUR} EUR je Stück sind Initialen inklusive, die Grafik kostet die Hälfte`) },
