            ...
            {hasImg ? (
              <>
                ...
                <div className="relative p-2.5 sm:p-3 h-full min-h-[120px] sm:min-h-[140px] flex flex-col justify-end">
                  ...
                  {sub && active && (
                    <div className="text-xs mt-0.5 drop-shadow-md tile-ink text-violet-200/90">
                      {sub}
                    </div>
                  )}
                </div>
              ...
              <div className={`p-3 sm:p-4 h-full min-h-[120px] sm:min-h-[140px] flex flex-col ${
                ...
                <div className={`text-xs sm:text-sm font-semibold leading-tight ${active ? "text-violet-200" : "text-white"}`}>
                  {label}
                </div>
                {sub && (
                  <div className={`text-xs sm:text-xs mt-0.5 ${active ? "text-violet-300" : "text-neutral-400"}`}>
                    {sub}
                  </div>
                )}
              </div>
...
function SimpleCard({ stepNum, label, children }) {
  return (
    <div className="rounded-xl border border-violet-400/10 bg-violet-400/[0.02] p-4 sm:p-5 mb-4">
      <div className="text-xs font-bold uppercase tracking-wider text-neutral-400 mb-3">
        {stepNum && <span className="text-violet-400 mr-1.5">{stepNum}</span>}{label}
      </div>
      {children}
    </div>
  );
...
export default function SimpleJewelryCalc({ lang = "pl" }) {
  ...
  return (
    <div>
      ...
      {/* Summary badge */}
      {!resolved?.custom && (
        <div className="mb-3 p-3 rounded-xl bg-violet-400/10 border border-violet-400/20 flex items-start gap-3">
          <Lightbulb className="w-4 h-4 text-violet-400 mt-0.5 flex-shrink-0" />
          <div className="text-[12px] leading-relaxed">
            <div className="text-violet-300 font-semibold mb-0.5">{l.summary}</div>
            <div className="text-violet-300">{paramsSummary}</div>
          </div>
        </div>
      )}
      ...
      <div className="mt-4 p-3 rounded-xl border border-violet-400/10 bg-violet-400/[0.02] text-xs text-violet-300 leading-relaxed text-center">
        {l.note}
      </div>
    </div>
  );
