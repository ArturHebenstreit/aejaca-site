              ...
              {TECHS.map(({ id, labelKey, descKey, img }) => {
                ...
                return (
                  <button key={id}
                    ...
                    className={`group relative rounded-xl border text-left transition-all duration-200 overflow-hidden min-h-[176px] ${
                    ...
                    {img && (
                      <div className="absolute inset-0 overflow-hidden">
                        <Obraz sizes="(min-width: 640px) 180px, 40vw" src={img} alt={l[labelKey]} loading="lazy"
                          className={`w-full h-full object-cover transition-all duration-500 ${active ? "tile-foto scale-105" : "tile-foto group-hover:scale-105"}`} />
                        <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/60 to-black/25" />
                      </div>
                    )}
                    {/* KAFELEK MA MIEC MIEJSCE NA ZDJECIE, a nie tylko na podpis. Przy
                        140 px i czterech linijkach opisu na fotografie zostawalo
                        okolo czterdziestu pikseli, wiec klient nie widzial, co
                        kafelek przedstawia (zgloszenie wlasciciela, 2026-09-04). */}
                    <div className="relative p-3 h-full min-h-[176px] flex flex-col justify-end">
