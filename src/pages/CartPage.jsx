              ...
              <div key={item.cartItemId} className="rounded-2xl border border-white/8 bg-white/[0.02] p-4">
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="space-y-1">
                    <p className="text-sm font-semibold text-white mb-2">{item.stlFilename || "Druk 3D"}</p>

                    {/* Material + segment */}
                    <p className="text-xs text-neutral-400">
                      <span className="text-neutral-500">Materiał:</span> {item.material}{item.segment ? ` (${item.segment})` : ""}
                    </p>

                    {/* Infill */}
                    {item.infill && (
                      <p className="text-xs text-neutral-400">
                        <span className="text-neutral-500">Wypełnienie:</span> {item.infill}
                      </p>
                    )}
                    ...
                    {item.precision && (
                      <p className="text-xs text-neutral-400">
                        <span className="text-neutral-500">Precyzja:</span> {item.precision}
                      </p>
                    )}
                    ...
                    {item.stlDims && (
                      <p className="text-xs text-neutral-400">
                        <span className="text-neutral-500">Wymiary modelu:</span> {item.stlDims.x}×{item.stlDims.y}×{item.stlDims.z} mm
                      </p>
                    )}
                    ...
                    {item.colors?.length > 0 && (
                      <div className="flex items-start gap-1.5 flex-wrap">
                        <span className="text-xs text-neutral-500 shrink-0">
                          {item.colors.length === 1 ? "Kolor:" : "Kolory:"}
                        </span>
                    ...
                    {item.colorDescriptions && (
                      <p className="text-xs text-neutral-400">
                        <span className="text-neutral-500">Przypisanie kolorów:</span> <span className="italic">{item.colorDescriptions}</span>
                      </p>
                    )}
                  </div>
                  <button
