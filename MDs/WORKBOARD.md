...
| TASK-026 | Audyt bryl kreatora: parzysty wieniec halo omijajacy krapy, pliki STL i 3MF bez zerowych trojkatow, minimum 0,45 mm dla srebra (obrecze, tulejki halo, kolnierze pod koszem i wokol gniazd w szynie, mostki i kuleczki pave) | Claude Code | `claude/serwis-development-skills-3vuba9` | `src/geometry/ring/build.js`, `src/geometry/ring/params.js`, `src/data/ringPresets.js`, `chat-api/geometry/`, `src/workers/ringGenerator.worker.js`, `chat-api/ringExport.js`, `chat-api/ringExport.test.mjs`, `chat-api/package.json`, `scripts/test-ring-generator.mjs`, `src/components/calculators/RingConfigurator.jsx`, `MDs/AEJaCA_Geometria_Kreatora_Zasady.md`, handoff | review | `MDs/handoffs/TASK-026-audyt-bryl-kreatora.md` |

| TASK-027 | Przyjecie modelu klienta do odlewu: deklaracja stanu pliku zamiast domyslanego skurczu, naddatek na szlif otworu, pomiar szczelnosci, liczby bryl, grubosci i srednicy otworu na serwerze | Claude Code | `claude/serwis-development-skills-3vuba9` | `src/data/castingSpec.js`, `src/pricing/castingIntake.js`, `src/pricing/preciousMetalCasting.js`, `src/analysis/printability.js`, `src/data/orderCatalog.js`, `src/components/shop/PolaUslugi.jsx`, `src/components/shop/ServiceConfigurator.jsx`, `src/components/calculators/MetalCastCalc.jsx`, `src/pages/Order.jsx`, `chat-api/orders.js`, `chat-api/server.js` (odpowiedz na wgranie pliku), `chat-api/pricing/`, `scripts/sync-pricing.mjs`, `scripts/test-casting-intake.mjs`, `package.json`, `public/llms.txt`, `chat-api/context.js`, `MDs/AEJaCA_Brand_Reference.md`, `src/data/termsContent.js`, `src/pages/OdlewZPliku.jsx`, `src/data/odlewZPliku.js`, `src/data/faq/odlewZPliku.js`, `src/data/faq/index.js`, `src/pages/Faq.jsx`, `src/routes.js`, `src/main.jsx`, `src/entry-server.jsx`, `src/seo/seoData.js`, `src/components/Footer.jsx`, `src/i18n/`, `public/_redirects`, `public/sitemap.xml`, ADR-0048 i handoff | review | `MDs/handoffs/TASK-027-model-klienta-do-odlewu.md` |

| TASK-028 | Zapytanie, o ktorym nikt sie nie dowiedzial: powiadomienie z ponowieniem i sladem w bazie, numer sprawy przed wyslaniem, nowe zapytania w porannym raporcie ponad lista zaleglosci | Claude Code | `claude/serwis-development-skills-3vuba9` | `chat-api/server.js` (migracja leads, `/api/contact`, `/api/quote`, `wyslijPowiadomienie`), `chat-api/podsumowanieDnia.js`, `scripts/test-podsumowanie-dnia.mjs`, handoff | review | `MDs/handoffs/TASK-028-zapytanie-bez-powiadomienia.md` |
