...
kalkulator, czat, mail na skrzynke), nadaje mu numer `WY` + data + osiem znakow.
Numer generuje **wylacznie** `quotes.js`; kopia z `server.js` znika.
...
przepisuje adres, jezyk, tresc, parametry i przyslany plik, zaklada wycene pod
numerem sprawy i przestawia zgloszenie na stan `quoted`, zeby nie dalo sie
zrobic z niego drugiej, sprzecznej oferty. Drugie klikniecie oddaje istniejaca
wycene zamiast zakladac blizniacza.
...
- **Mail przychodzacy zaklada sprawe z numerem i z trescia.** Wczesniej
  zakladal zgloszenie z samym tematem.

## Niezmienniki i testy

- Kazde miejsce zapisujace zgloszenie nadaje numer. Test przeglada WSZYSTKIE
  wstawki do `leads` w `server.js` i `gmail.js` i wymaga kolumny `quote_ref`:
  nowa droga dopisana bez numeru zapali sie od razu, a nie przy odpisywaniu na
  zapytanie. Test: `chat-api/numerSprawy.test.mjs`.
